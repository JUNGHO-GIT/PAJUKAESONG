...
export const FranchiseInfo = () => {
  ...
  const infoNode = () => {
    ...
    const infoSection = () => {
      ...
      return (
        <Grid container spacing={0} columns={12} className={"border-1 radius-1 shadow-1"}>
          <Grid size={{ xs: 12, sm: 12, md: 12, lg: 12, xl: 12 }} key={`info-${0}`}>
            {titleFragment(OBJECT_INFO)}
            {infoFragment(OBJECT_INFO)}
          </Grid>
        </Grid>
      );
    };
    ...
    const stepSection = () => {
          ...
          {isXs ? (
            <Grid size={12}>
              <Stepper
                ...
                className={"w-100p"}
              >
                {item.stepArray.map((step: any, index: number) => (
                  <Step key={index}>
                    <StepLabel
                      icon={index + 1}
                      className={"pointer"}
            ...
            <Grid size={12}>
              <Stepper
                ...
                className={"w-100p"}
              >
                {item.stepArray.slice(0, 4).map((step: any, index: number) => (
                  <Step key={index}>
                    <StepLabel
                      icon={index + 1}
                      className={"pointer"}
              ...
              <Stepper
                ...
                className={"w-100p"}
              >
                {item.stepArray.slice(4, 8).map((step: any, index: number) => (
                  <Step key={index}>
                    <StepLabel
                      icon={index + 5}
                      className={"pointer"}
      ...
      return (
        <Grid container spacing={0} columns={12} className={"border-1 radius-1 shadow-1"}>
          <Grid size={{ xs: 12, sm: 12, md: 12, lg: 12, xl: 12 }} key={`step-${0}`}>
            {titleFragment(OBJECT_STEP)}
    ...
    const costSection = () => {
      ...
      const headerFragment = (item: any) => (
        <Grid container spacing={2} columns={12} className={"p-20 border-bottom-1-burgundy"}>
          <Grid size={3} className={"d-center"}>
            <Div className={"fs-0-8rem fw-500"}>
              {item.constHeader.label}
            </Div>
          </Grid>
          <Grid size={6} className={"d-center"}>
            <Div className={"fs-0-8rem fw-500"}>
              {item.constHeader.desc}
            </Div>
          </Grid>
          <Grid size={3} className={"d-center"}>
            <Div className={"fs-0-8rem fw-500"}>
              {item.constHeader.cost}
            </Div>
          </Grid>
        </Grid>
      );
      const costFragment = (item: any) => (
        item.constArray.map((step: any, index: number) => (
          <Grid container spacing={2} columns={12} className={"p-20 border-bottom-1"} key={index}>
            <Grid size={3}>
              <Div className={"fs-0-8rem"}>
                {step.label}
              </Div>
      ...
      return (
        <Grid container spacing={0} columns={12} className={"border-1 radius-1 shadow-1"}>
          <Grid size={{ xs: 12, sm: 12, md: 12, lg: 12, xl: 12 }} key={`cost-${0}`}>
            {titleFragment(OBJECT_COST)}
            {headerFragment(OBJECT_COST)}
            {costFragment(OBJECT_COST)}
          </Grid>
        </Grid>
      );
    };
